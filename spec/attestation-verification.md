...
### 3.1 Key-attestation chain to the Google root (offline, feature `keyattest`)

The leaf in `certificate_chain` (field 3) is what makes the field-2 binding
trustworthy: its signature establishes that the device key is genuine TEE /
StrongBox hardware. That is validated offline by walking the chain to a Google
hardware-attestation root:

1. **Parse** every certificate in `certificate_chain[]` (leaf first).
2. **Validity.** Each certificate must be within its `notBefore`..`notAfter`
   window at verification time.
3. **Linkage.** Each certificate's signature must verify under the next
   certificate's public key. Algorithms accepted: RSA PKCS#1 v1.5 (SHA-256/384)
   and ECDSA (SHA-256/384) over a P-256 or P-384 issuer key — covering both the
   long-standing RSA-4096 Google root and the ECDSA P-384 `Key Attestation CA1`
   root effective 2026-02-01.
4. **Anchor.** The top of the supplied chain must be, or be signed by, one of the
   two Google roots embedded in the crate and pinned by SHA-256 fingerprint.
5. **Leaf attestation extension** (OID `1.3.6.1.4.1.11129.2.1.17`). Parse the
...
consult it, so a key revoked *after* issuance is not detected here. The deeper
`RootOfTrust` / verified-boot fields of the authorization lists are likewise not
yet enforced — `attestationSecurityLevel` already establishes hardware backing.
